...
// ════════════════════AUTO BIO◽◽◽◽◽    
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var announce = ''
            if (config.LANG == 'EN') announce = '📢◉◉👩Emo◉◉ \n👾Announcement SYSTEM 🔘'
            if (config.LANG == 'SI') announce = '📢◉◉👩‍🦰Emo◉◉ \n👾නිවේදන පද්ධතිය 🔘'
            if (config.LANG == 'ID') announce = '📢◉◉👩‍🦰Emo◉◉ \n👾Announcement System 🔘'
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Amazone.png"), MessageType.image, { caption: '*🚀Emo*  WORKING PERFECTLY !!\n\n▷ _This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n_👾Support 01▷ Https://t.me/Emo_Bot_Support\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Amazone.png"), MessageType.image, { caption: '*🚀Emo*  නිසි ලෙස ක්‍රියා කරයි!!\n\n▷ _මෙය ඔබගේ LOG අංකයයි මෙහි විධන භාවිතයෙන් වළකින්න_\n▷ඔබට යම් ගැටලුවක් ඇත්නම් අපගේ සහය සමූහට ලිවිය හැක.\n_Support 01> Https://t.me/Emo_Bot_Support\n\n*දෝෂය:* ```' + error + '```\n\n' });
